...
@router.get("/", response_model=list[schemas.BoardWithPreviews])
def get_boards(
...
@router.get("/{id}", response_model=schemas.BoardWithPosts)
def get_board_by_id(id: int, db: Session=Depends(db.get_db)):
    return BoardControllers.get_board_by_id(id, db)

@router.post("/", response_model=schemas.BoardWithPreviews)
...
@router.put("/{id}", response_model=schemas.BoardWithPreviews)
